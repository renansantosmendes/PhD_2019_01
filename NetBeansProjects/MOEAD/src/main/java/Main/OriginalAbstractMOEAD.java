 ...
 * @author renansantos
 */
public abstract class OriginalAbstractMOEAD<S extends Solution<?>> implements Algorithm<List<S>> {
    ...
    public OriginalAbstractMOEAD(Problem<S> problem, int populationSize, int resultPopulationSize,
...
//initialize the nadir point
    ...
    double fitnessFunction(S individual, double[] lambda) throws JMetalException {
        ...
        if (org.uma.jmetal.algorithm.multiobjective.moead.MOEAD.FunctionType.TCHE.equals(functionType)) {
            ...
            fitness = maxFun;
        } else if (org.uma.jmetal.algorithm.multiobjective.moead.MOEAD.FunctionType.AGG.equals(functionType)) {
            double sum = 0.0;
            for (int n = 0; n < problem.getNumberOfObjectives(); n++) {
        ...
        } else if (org.uma.jmetal.algorithm.multiobjective.moead.MOEAD.FunctionType.PBI.equals(functionType)) {
            double d1, d2, nl;
            double theta = 5.0;
