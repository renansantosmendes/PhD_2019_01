...
public class OVO_SVM_RFE extends SVMBasedMethods {
    ...
    /**
     * generates binary classifiers (SVM by applying One-Versus-One (OVO)
    ...
    private double[] getFeaturesWeights(int[] selectedFeature) {
        ...
        double[][][] sparseWeights = buildSVM_OneAgainstOne(selectedFeature);
        int numClassifers = numClass * (numClass - 1) / 2;
        double[][] weights = new double[numClassifers][selectedFeature.length];
        int countClassifier = 0;

        for (int i = 0; i < numClass; i++) {
            for (int j = i + 1; j < numClass; j++) {
                for (int k = 0; k < selectedFeature.length; k++) {
                    weights[countClassifier][k] = sparseWeights[i][j][k];
                }
                countClassifier++;
            }
        }
     ...
     * based on recursive feature elimination using One-Versus-One strategy
     * (OVO_SVM_RFE)
     */
    @Override
