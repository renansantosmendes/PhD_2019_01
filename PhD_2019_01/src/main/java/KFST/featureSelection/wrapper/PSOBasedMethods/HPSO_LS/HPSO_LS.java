 ...
 */
public class HPSO_LS extends BasicPSO<Swarm> {
     ...
     * <code>epsilon</code>, <code>alpha</code>) in which
     ...
     * subset size determining scheme, and <code><b><i>alpha</i></b></code> is
     * the alpha parameter used in the local search operation
     */
    public HPSO_LS(Object... arguments) {
        super(arguments);
        swarm = new Swarm((double) arguments[14], (double) arguments[15]);
    }
    ...
    /**
     * starts the feature selection process by hybrid particle swarm
     * optimization method using local search (HPSO-LS)
     */
    @Override
    public void evaluateFeatures() {
        Swarm.fitnessEvaluator.createTempDirectory();
        Swarm.fitnessEvaluator.setDataInfo(trainSet, nameFeatures, classLabel);
        swarm.setDataInfo(trainSet);
